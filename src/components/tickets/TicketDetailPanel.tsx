        ...
        <div className="flex flex-wrap gap-2">
          {attachments.map((a) => (
            <Badge key={a.url} variant="secondary" className="gap-1 pr-1">
              ...
              {a.name}
              <button onClick={() => removeAttachment(a.url)} className="ml-1 rounded-full p-0.5 hover:bg-destructive/20">
          ...
          <>
            {hasReply ? (
              ...
              <Button size="sm" onClick={() => setIsComposing(true)}>
                <Edit className="mr-1.5 h-3.5 w-3.5" /> Compose Reply
              </Button>
            )}
            {hasReply && (
            ...
            <Button variant="ghost" size="sm" onClick={onIgnore} disabled={isPending}>
...
export function TicketDetailPanel({
  ...
  const handleAutoFetch = useCallback(async () => {
    ...
    try {
      ...
      if (accounts.some((a) => a.provider === "gmail" && a.is_active)) {
        fetches.push(supabase.functions.invoke("fetch-gmail-emails", { headers: { Authorization: `Bearer ${session.access_token}` } }));
      }
      if (accounts.some((a) => a.provider === "imap" && a.is_active)) {
        fetches.push(supabase.functions.invoke("fetch-imap-emails", { headers: { Authorization: `Bearer ${session.access_token}` } }));
      }
    ...
    try {
      ...
      for (const result of results) {
        if (result.status === "fulfilled" && !result.value.error) {
          totalProcessed += result.value.data?.processed || 0;
        }
      }
      await queryClient.invalidateQueries({ queryKey: ["ticket-emails"] });
      await queryClient.invalidateQueries({ queryKey: ["email-queue"] });
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-border px-6 py-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            >
              {autoFetchEnabled ? <Pause className="mr-1 h-3 w-3" /> : <Play className="mr-1 h-3 w-3" />}
              {autoFetchEnabled ? "Stop" : "Auto"}
            </Button>
      ...
      <div className="flex border-b border-border">
        ...
        >
          ...
          {pendingCount > 0 && (
            <Badge variant="secondary" className="ml-1.5 h-5 px-1.5 text-[10px] bg-destructive/10 text-destructive">
                    ...
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-1.5">
                          ...
                          {isPendingEmail && (
                            <Badge className="bg-yellow-500/10 text-yellow-600 text-xs">Needs Review</Badge>
                          )}
                        </div>
                      </div>
                      ...
                      {email.suggested_reply && !isPendingEmail && (
                        <div className="rounded-md border border-primary/20 bg-primary/5 p-3">
                          <p className="text-xs font-medium text-muted-foreground mb-1.5 flex items-center gap-1">
                            <Send className="h-3 w-3" /> {isSent ? "Reply Sent" : "AI Reply"}
                          </p>
                          <p className="whitespace-pre-wrap text-sm text-foreground leading-relaxed">
                      ...
                      {/* Pending email: show suggested reply + action buttons */}
                      {isPendingEmail && (
                        <>
                          {email.suggested_reply && (
                            <div className="rounded-md border border-primary/20 bg-primary/5 p-3">
                              <p className="text-xs font-medium text-muted-foreground mb-1.5 flex items-center gap-1">
                                <Send className="h-3 w-3" /> AI Suggested Reply
                              </p>
                              <p className="whitespace-pre-wrap text-sm text-foreground leading-relaxed">
                                {email.suggested_reply}
                              </p>
