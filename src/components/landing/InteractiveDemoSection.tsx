        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-10 lg:gap-8">
          {steps.map((s, i) => (
            <div key={s.num} className="relative">
              <span className="text-[48px] font-semibold text-zinc-700 leading-none block mb-3 tracking-tighter">{s.num}</span>
              ...
              {i < steps.length - 1 && (
                <span className="hidden lg:block absolute top-6 -right-5 text-zinc-600 text-lg">→</span>
              )}
            </div>
          ))}
        </div>
